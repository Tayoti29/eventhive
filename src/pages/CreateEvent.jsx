...
function CreateEvent() {
  ...
  const handleDrop = (e) => {
    e.preventDefault()
    handleFlyer(e.dataTransfer.files[0])
  }
  ...
  const handleConfirmUpload = async () => {
    ...
    try {
      ...
      // Build event object — only use UUID if real Supabase user
      const eventData = {
        title: form.title,
        ...
        is_free: form.fee === 'free',
        ticket_price: form.fee === 'paid' ? form.feeAmount : null,
        image_url: imageUrl,
    ...
    <div style={{ backgroundColor: '#F5F4F0', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '32px 100px 80px 100px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '40px' }}>
          <h1 style={{ fontSize: '40px', fontWeight: '700', color: '#141415', margin: 0 }}>Create Event</h1>
        ...
        <div style={{ maxWidth: '540px', margin: '0 auto' }}>
          ...
          {!flyerPreview ? (
            ...
            >
              ...
              <p style={{ fontSize: '12px', color: '#A5A5AA', margin: 0, textAlign: 'center' }}>
                Click to choose file or drag and drop the file here.<br />File can be jpg, png. File not more than 5MB
              </p>
              ...
              {showImageMenu && (
                <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', display: 'flex', flexDirection: 'column', gap: '8px', zIndex: 10 }}>
                  <button onClick={() => setShowFullImage(true)}
                    style={{ padding: '8px 20px', borderRadius: '8px', backgroundColor: '#0097FF', color: '#FFFFFF', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: '600', whiteSpace: 'nowrap' }}>
                    + View Image
                  </button>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <textarea placeholder="Write event description..." maxLength={200} value={form.description}
              ...
              rows={4}
              style={{ width: '100%', borderRadius: '8px', border: '1px solid #E8E8EA', padding: '10px 14px', fontSize: '14px', color: '#414143', outline: 'none', boxSizing: 'border-box', resize: 'none', fontFamily: 'inherit' }} />
            <div style={{ textAlign: 'right' }}>
              <span style={{ fontSize: '11px', color: '#A5A5AA' }}>{form.description.length}/200</span>
            </div>
          </div>
          ...
          <div style={{ marginBottom: '16px', position: 'relative' }}>
            ...
            <button
              ...
              <span style={{ color: form.eventType ? '#414143' : '#A5A5AA' }}>
                {form.eventType
                  ? (eventTypes.find((t) => t.label === form.eventType)?.icon + ' ' + form.eventType)
                  : 'Select Event Type'}
              </span>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#7E7E82" strokeWidth="2"
                style={{ transform: showTypeFlyout ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s ease' }}>
            ...
            {showTypeFlyout && (
              <div style={{ position: 'absolute', top: '76px', left: 0, width: '100%', backgroundColor: '#FFFFFF', borderRadius: '12px', boxShadow: '0 8px 32px rgba(0,0,0,0.12)', padding: '8px', zIndex: 100, maxHeight: '240px', overflowY: 'auto' }}>
                <p style={{ fontSize: '12px', fontWeight: '600', color: '#7E7E82', padding: '4px 8px', margin: '0 0 4px 0' }}>Select Event Type</p>
                {eventTypes.map((type) => (
                  <div key={type.label}
                    onClick={() => { setForm((prev) => ({ ...prev, eventType: type.label })); setShowTypeFlyout(false) }}
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: '12px' }}>
              {['physical', 'online', 'both'].map((v) => (
                <button key={v} onClick={() => setForm((prev) => ({ ...prev, venueType: v }))}
                  style={{ height: '40px', borderRadius: '8px', border: `1px solid ${form.venueType === v ? '#0097FF' : '#E8E8EA'}`, backgroundColor: form.venueType === v ? '#EFF9FF' : '#FFFFFF', color: form.venueType === v ? '#0097FF' : '#59595C', fontSize: '13px', fontWeight: '500', cursor: 'pointer' }}>
            ...
            {(form.venueType === 'physical' || form.venueType === 'both') && (
              <div style={{ marginBottom: '8px' }}>
                <input type="text" placeholder="E.g Ikoyi Beach, Lagos"
                  value={form.address}
            ...
            {(form.venueType === 'online' || form.venueType === 'both') && (
              <div>
                <input type="text" placeholder="Enter registration or online link"
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <select value={form.eventDays}
              onChange={(e) => setForm((prev) => ({ ...prev, eventDays: e.target.value }))}
              ...
              {form.eventDays === 'one' ? (
                ...
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: '8px', alignItems: 'center', marginBottom: '12px' }}>
                    ...
                    <input type="date" value={form.dateRange}
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
              {['free', 'paid'].map((f) => (
                <button key={f} onClick={() => setForm((prev) => ({ ...prev, fee: f }))}
                  style={{ height: '36px', padding: '0 20px', borderRadius: '8px', border: `1px solid ${form.fee === f ? '#0097FF' : '#E8E8EA'}`, backgroundColor: form.fee === f ? '#EFF9FF' : '#FFFFFF', color: form.fee === f ? '#0097FF' : '#59595C', fontSize: '13px', fontWeight: '500', cursor: 'pointer' }}>
                  {f === 'free' ? '🆓 Free' : '💰 Paid'}
                </button>
              ))}
            </div>
            {form.fee === 'paid' && (
              <input type="text" placeholder="Enter ticket price e.g ₦2,000"
          ...
          <div style={{ marginBottom: '32px' }}>
            ...
            <div style={{ position: 'relative' }}>
              <input type="text" placeholder="Type a tag and press Enter, or pick from suggestions"
                value={tagInput}
                onChange={(e) => handleTagInput(e.target.value)}
                onFocus={() => { handleTagInput(tagInput); setShowTagSuggestions(true) }}
                onBlur={() => setTimeout(() => setShowTagSuggestions(false), 200)}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && tagInput.trim() && form.tags.length < 3) {
                    e.preventDefault()
              ...
              {showTagSuggestions && tagSuggestions.length > 0 && (
                <div style={{ position: 'absolute', top: '48px', left: 0, width: '100%', backgroundColor: '#FFFFFF', borderRadius: '8px', boxShadow: '0 4px 16px rgba(0,0,0,0.1)', padding: '8px', zIndex: 100 }}>
                  <p style={{ fontSize: '11px', color: '#A5A5AA', margin: '0 0 6px 4px' }}>
                    Suggestions — {form.tags.length} of 3 selected
                  </p>
                ...
                {form.tags.map((tag) => (
                  <div key={tag} style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '4px 12px', borderRadius: '9999px', backgroundColor: '#EFF9FF', border: '1px solid #0097FF' }}>
                    ...
                    <button onClick={() => removeTag(tag)}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#0097FF', padding: 0, lineHeight: 1, fontSize: '16px' }}>×</button>
                  </div>
                ))}
              </div>
            )}
      ...
      {/* Full Image Modal */}
      {showFullImage && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      {showUploadConfirm && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ width: '420px', backgroundColor: '#FFFFFF', borderRadius: '16px', padding: '32px', boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }}>
            <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#141415', marginBottom: '8px' }}>
              You are uploading {form.title}
            </h3>
