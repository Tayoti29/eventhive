...
function RelatedMemeAdCard({ ad, isMobile }) {
  const [hovered, setHovered] = useState(false)
  const content = (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
...
function CaptionItem({ caption, isMobile }) {
  ...
  const showCopyButton = isMobile || hovered

  const doCopy = () => {
    navigator.clipboard.writeText(text + ' — ' + authorName)
    setCopied(true)
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ display: 'flex', gap: isMobile ? '8px' : '10px', padding: isMobile ? '12px' : '14px 16px', borderRadius: '8px', border: '1px solid #E8E8EA', backgroundColor: hovered ? '#F9F9F9' : '#FFFFFF', marginBottom: isMobile ? '10px' : '12px', position: 'relative', transition: 'background-color 0.2s ease' }}>
      <UserAvatar avatarUrl={authorAvatar} name={authorName} size={isMobile ? 24 : 28} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontSize: isMobile ? '11px' : '12px', color: '#A5A5AA', marginBottom: '4px', fontWeight: '500' }}>{authorName}</p>
        <p style={{ fontSize: isMobile ? '13px' : '15px', color: '#141415', lineHeight: isMobile ? '19px' : '22px', margin: 0, paddingRight: showCopyButton ? (isMobile ? '34px' : '80px') : '0' }}>{text}</p>
      </div>
      {showCopyButton && (
        <button onClick={doCopy}
          style={{ position: 'absolute', right: isMobile ? '10px' : '12px', top: isMobile ? '10px' : '50%', transform: isMobile ? 'none' : 'translateY(-50%)', display: 'flex', flexDirection: isMobile ? 'row' : 'column', alignItems: 'center', gap: '2px', background: 'none', border: 'none', cursor: 'pointer' }}>
          <svg width={isMobile ? '16' : '18'} height={isMobile ? '16' : '18'} viewBox="0 0 24 24" fill="none" stroke="#0097FF" strokeWidth="2">
            <rect x="9" y="9" width="13" height="13" rx="2" ry="2"/>
            <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/>
          </svg>
          {!isMobile && <span style={{ fontSize: '10px', color: '#0097FF', whiteSpace: 'nowrap' }}>{copied ? 'Copied!' : 'Copy caption'}</span>}
        </button>
      )}
      {isMobile && copied && (
        <span style={{ position: 'absolute', right: '10px', bottom: '10px', fontSize: '10px', color: '#2E7D32', fontWeight: '600' }}>Copied!</span>
      )}
    </div>
  )
}

function RelatedMemeCard({ meme, isMobile }) {
  const [hovered, setHovered] = useState(false)
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      style={{ borderRadius: isMobile ? '10px' : '12px', overflow: 'hidden', cursor: isValidUUID(meme.id) ? 'pointer' : 'default', boxShadow: hovered ? '0 8px 24px rgba(0,0,0,0.15)' : '0 2px 8px rgba(0,0,0,0.08)', transition: 'all 0.3s ease', transform: hovered ? 'translateY(-4px)' : 'translateY(0)', backgroundColor: '#FFFFFF' }}>
      <div style={{ position: 'relative', height: isMobile ? '150px' : '220px' }}>
      ...
      <div style={{ padding: isMobile ? '8px' : '10px 12px', backgroundColor: hovered ? '#F9F9F9' : '#FFFFFF', transition: 'background-color 0.3s ease' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '5px' : '6px', marginBottom: isMobile ? '5px' : '6px' }}>
          <UserAvatar avatarUrl={meme.uploader_avatar} name={meme.uploader_name} size={isMobile ? 18 : 22} />
          <span style={{ fontSize: isMobile ? '10px' : '11px', color: '#7E7E82', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{meme.uploader_name || 'User'}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '8px' : '10px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
            <svg width={isMobile ? '11' : '12'} height={isMobile ? '11' : '12'} viewBox="0 0 24 24" fill="none" stroke="#7E7E82" strokeWidth="2">
          ...
          <button onClick={(e) => { e.stopPropagation(); if (!liked) { setLiked(true); setLikes(likes + 1) } }}
            ...
            <svg width={isMobile ? '11' : '12'} height={isMobile ? '11' : '12'} viewBox="0 0 24 24" fill={liked ? '#D90870' : 'none'} stroke="#D90870" strokeWidth="2">
              <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
            </svg>
            <span style={{ fontSize: isMobile ? '10px' : '11px', color: liked ? '#D90870' : '#7E7E82' }}>{likes}</span>
          </button>
        </div>
      </div>
    </div>
...
function MemeDetail() {
  ...
  const fetchMeme = async () => {
    ...
    try {
      ...
      const { data: caps } = await supabase.from('meme_captions').select('*').eq('meme_id', id).order('created_at', { ascending: false })
      setCaptions(caps || [])

      const { data: related } = await supabase.from('memes').select('*').eq('category', data.category).neq('id', id).limit(9)
  ...
  const handleAddCaption = async () => {
    ...
    const tempCaption = { id: 'temp-' + Date.now(), author_name: author.name, author_avatar: author.avatar, caption: newCaption }
    setCaptions([tempCaption, ...captions])
    setNewCaption('')

    try {
      await supabase.from('meme_captions').insert([{
        meme_id: id, caption: newCaption, author_name: author.name, author_avatar: author.avatar,
      ...
      <div style={{ backgroundColor: '#FFFFFF', minHeight: '100vh' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', padding: '120px 0' }}>
          <div style={{ display: 'flex', gap: '8px' }}>
            {[0,1,2].map((i) => <div key={i} style={{ width: '12px', height: '12px', borderRadius: '9999px', backgroundColor: '#D90870', animation: 'bounce 0.6s ease ' + (i * 0.1) + 's infinite alternate' }} />)}
          </div>
        </div>
  ...
  const relatedColumns = Array.from({ length: numCols }, () => [])
  relatedItemsWithAds.forEach((item, i) => relatedColumns[i % numCols].push(item))
  ...
  return (
    <div style={{ backgroundColor: '#FFFFFF' }}>
          ...
          <>
            ...
            <div style={{ marginTop: '18px', marginBottom: '14px' }}>
              ...
              <p style={{ fontSize: '11px', color: '#A5A5AA', margin: 0 }}>
                Uploaded on {new Date(m.created_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
              </p>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '4px 0 16px' }}>
              ...
              <div style={{ flex: 1 }} />
              <button onClick={handleDownloadMeme} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
              ...
              <div>
                <p style={{ fontSize: '13px', fontWeight: '700', color: '#141415', margin: 0 }}>{m.uploader_name || 'User'}</p>
                <p style={{ fontSize: '11px', color: '#7E7E82', margin: 0 }}>Uploader</p>
              </div>
            </div>
            ...
            <div style={{ marginBottom: '16px' }}>
              {displayCaptions.map((cap) => <CaptionItem key={cap.id} caption={cap} isMobile />)}
            </div>
            ...
            <div style={{ position: 'relative', marginBottom: '4px' }}>
              <input type="text"
                ...
                style={{ width: '100%', height: '46px', borderRadius: '10px', border: '1px solid #E8E8EA', padding: '0 46px 0 14px', fontSize: '13px', color: '#414143', backgroundColor: user ? '#FFFFFF' : '#F9F9F9', outline: 'none', boxSizing: 'border-box', cursor: user ? 'text' : 'pointer' }}
              />
              {user && (
                <button onClick={handleAddCaption} style={{ position: 'absolute', right: '10px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer' }}>
            ...
            <div style={{ marginBottom: '28px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {memeCategories.map((cat) => {
                  ...
                  return (
                    <button key={cat} onClick={() => navigate('/memes', { state: { category: cat } })}
                      style={{ padding: '6px 14px', borderRadius: '9999px', border: '1px solid ' + (isActive ? '#0097FF' : '#E8E8EA'), backgroundColor: isActive ? '#0097FF' : '#FFFFFF', color: isActive ? '#FFFFFF' : '#59595C', fontSize: '12px', fontWeight: '500', cursor: 'pointer' }}>
                      {cat}
                    </button>
                  )
                })}
              ...
              <div style={{ marginBottom: '32px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  {relatedItemsWithAds.map((item, idx) =>
                    item.kind === 'ad'
                      ? <RelatedMemeAdCard key={'ad-' + idx} ad={item.data} isMobile />
                      : <RelatedMemeCard key={item.data.id} meme={item.data} isMobile />
                  )}
                </div>
                <button onClick={() => navigate('/memes')}
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <IconButton hoverText="Download" onClick={handleDownloadMeme}
                    ...
                    <IconButton hoverText="Share Meme" onClick={() => setShowShareModal(true)}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 292px', gap: '48px', marginTop: '48px' }}>
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  />
                  {user && (
                    <button onClick={handleAddCaption} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer' }}>
                      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#0097FF" strokeWidth="2"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
                    </button>
                  )}
                </div>
              </div>
      ...
      {showShareModal && (
        <ShareModal shareUrl={shareUrl} shareTitle={shareTitle} onClose={() => setShowShareModal(false)} isMobile={isMobile} />
      )}
