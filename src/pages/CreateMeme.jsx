...
const memeCategories = [
  { label: 'Funny', icon: '🤣' }, { label: 'Relatable', icon: '😅' }, { label: 'Motivational', icon: '💪' },
  { label: 'Naija', icon: '🇳🇬' }, { label: 'Couple Goals', icon: '❤️' }, { label: 'Work Life', icon: '💼' },
  { label: 'School', icon: '📚' }, { label: 'Politics', icon: '🏛️' }, { label: 'Sports', icon: '⚽' },
]
...
function CreateMeme() {
  ...
  const addCaption = () => { if (captions.length < 5) setCaptions([...captions, '']) }
  const updateCaption = (index, value) => { const updated = [...captions]; updated[index] = value; setCaptions(updated) }
  const removeCaption = (index) => { if (captions.length === 1) return; setCaptions(captions.filter((_, i) => i !== index)) }
  ...
  const handleUpload = () => { if (validate()) setShowUploadConfirm(true) }
  ...
  const handleConfirmUpload = async () => {
    ...
    try {
      ...
      if (flyer) {
        ...
        const { error: uploadError } = await supabase.storage.from('meme-images').upload(fileName, flyer, { contentType: flyer.type })
        if (!uploadError) {
          const { data: urlData } = supabase.storage.from('meme-images').getPublicUrl(fileName)
          imageUrl = urlData.publicUrl
        }
      }
      const memeData = {
        title: captions[0].slice(0, 80), caption: captions[0], category, image_url: imageUrl,
        ...
        likes: 0, saves: 0, views: 0, downloads: 0,
      }
      const { data: insertedMeme } = await supabase.from('memes').insert([memeData]).select()
      const memeId = insertedMeme?.[0]?.id
      if (memeId) {
        const captionInserts = captions.filter((c) => c.trim()).map((cap) => ({
          meme_id: memeId, caption: cap,
          author_name: user?.user_metadata?.full_name || user?.email?.split('@')[0] || 'Anonymous',
          user_id: isValidUUID(user?.id) ? user.id : null,
        }))
        await supabase.from('meme_captions').insert(captionInserts)
      }
  ...
  return (
    <div style={{ backgroundColor: '#F5F4F0', minHeight: '100vh' }}>
      {showSuccess && <SuccessBanner message="Your meme has been uploaded successfully 😂" onDone={() => navigate('/my-uploads')} />}
      ...
      <div style={{ maxWidth: '1440px', margin: '0 auto', padding: isMobile ? '20px 20px 60px' : '32px 100px 80px 100px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'stretch' : 'center', justifyContent: 'space-between', gap: isMobile ? '14px' : 0, marginBottom: isMobile ? '24px' : '40px' }}>
          <h1 style={{ fontSize: isMobile ? '26px' : '40px', fontWeight: '700', color: '#141415', margin: 0 }}>Upload Meme</h1>
        ...
        <div style={{ maxWidth: '540px', margin: '0 auto' }}>
          ...
          {!flyerPreview ? (
            <div onClick={() => fileInputRef.current?.click()}
              ...
              <p style={{ fontSize: '12px', color: '#A5A5AA', margin: 0, textAlign: 'center' }}>Click to upload. JPG, PNG, GIF. Max 5MB</p>
            ...
            <div style={{ position: 'relative', marginBottom: '24px' }}
              ...
              <img src={flyerPreview} alt="Meme" style={{ width: '100%', borderRadius: '12px', display: 'block', maxHeight: isMobile ? '260px' : '300px', objectFit: 'contain', backgroundColor: '#F3F3F4' }} />
              {showImageMenu && (
          ...
          <input ref={fileInputRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={(e) => handleFlyer(e.target.files[0])} />
          <input ref={replaceInputRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={(e) => handleFlyer(e.target.files[0])} />
          ...
          <div style={{ marginBottom: '16px', position: 'relative' }}>
            ...
            <button onClick={() => setShowCategoryFlyout(!showCategoryFlyout)}
              ...
              <span style={{ color: category ? '#414143' : '#A5A5AA' }}>{category ? (memeCategories.find((c) => c.label === category)?.icon + ' ' + category) : 'Select Category'}</span>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#7E7E82" strokeWidth="2" style={{ transform: showCategoryFlyout ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s ease' }}><polyline points="6 9 12 15 18 9"/></svg>
            </button>
            {errors.category && <span style={{ fontSize: '12px', color: '#AE2012', display: 'block', marginTop: '4px' }}>{errors.category}</span>}
            {showCategoryFlyout && (
              <div style={{ position: isMobile ? 'fixed' : 'absolute', top: isMobile ? undefined : '76px', bottom: isMobile ? 0 : undefined, left: 0, right: isMobile ? 0 : undefined, width: '100%', borderRadius: isMobile ? '20px 20px 0 0' : '12px', backgroundColor: '#FFFFFF', boxShadow: '0 8px 32px rgba(0,0,0,0.12)', padding: '8px', zIndex: 200, maxHeight: isMobile ? '55vh' : 'none', overflowY: 'auto' }}>
                {memeCategories.map((cat) => (
                  <div key={cat.label} onClick={() => { setCategory(cat.label); setShowCategoryFlyout(false) }}
          ...
          <div style={{ marginBottom: '32px' }}>
            ...
            {captions.map((caption, i) => (
              <div key={i} style={{ display: 'flex', gap: '8px', marginBottom: '10px', alignItems: 'center' }}>
                <input type="text" placeholder={`Caption ${i + 1}${i === 0 ? ' (required)' : ' (optional)'}`} value={caption}
                  onChange={(e) => updateCaption(i, e.target.value)}
                  style={{ ...inputStyle, flex: 1, borderColor: i === 0 && errors.caption ? '#AE2012' : '#E8E8EA' }} />
                {captions.length > 1 && (
                  <button onClick={() => removeCaption(i)}
                    style={{ width: isMobile ? '46px' : '40px', height: isMobile ? '46px' : '44px', borderRadius: '8px', border: '1px solid #E8E8EA', backgroundColor: '#FFFFFF', cursor: 'pointer', color: '#AE2012', fontSize: '18px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
                )}
              </div>
            ))}
            {errors.caption && <span style={{ fontSize: '12px', color: '#AE2012', display: 'block', marginBottom: '8px' }}>{errors.caption}</span>}
            {captions.length < 5 && (
              <button onClick={addCaption} style={{ width: isMobile ? '100%' : 'auto', height: '42px', padding: '0 20px', borderRadius: '8px', border: '2px dashed #C7C7CA', backgroundColor: '#FFFFFF', cursor: 'pointer', fontSize: '13px', color: '#59595C', marginTop: '4px' }}>
                + Add Another Caption
              </button>
            )}
          </div>
      ...
      {showUploadConfirm && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: isMobile ? '0 24px' : 0 }}>
          <div style={{ width: isMobile ? '100%' : '420px', backgroundColor: '#FFFFFF', borderRadius: '16px', padding: isMobile ? '24px' : '32px', boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }}>
            ...
            <p style={{ fontSize: '14px', color: '#7E7E82', marginBottom: '24px' }}>Are you sure you want to upload this meme?</p>
