...
function Login() {
  ...
  const handleEmailLogin = async () => {
    ...
    try {
      ...
      if (error) {
        // Fallback: mock login with valid UUID
        console.log('Supabase login failed, using mock login:', error.message)
        setMockUser(email)
        setShowSuccess(true)
      } else {
        setUser(data.user)
  ...
  return (
    <>
      {showSuccess && (
        <SuccessBanner message="You have logged in successfully" onDone={handleClose} />
      )}
        ...
        {phase === 'choose' && (
          <div style={{ width: '449px', backgroundColor: '#FFFFFF', borderRadius: '24px', padding: '40px', position: 'relative', boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }}>
            <button onClick={handleClose}
              style={{ position: 'absolute', top: '20px', right: '20px', background: 'none', border: 'none', cursor: 'pointer', color: '#7E7E82', fontSize: '20px' }}>✕</button>
        ...
        {phase === 'email' && (
          <div style={{ width: '449px', backgroundColor: '#FFFFFF', borderRadius: '24px', padding: '40px', position: 'relative', boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '32px' }}>
              ...
              <button onClick={handleClose}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#7E7E82', fontSize: '20px' }}>✕</button>
            </div>

            <h3 style={{ fontSize: '32px', lineHeight: '39px', fontWeight: '700', color: '#141415', textAlign: 'center', marginBottom: '32px' }}>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <input type="email" placeholder="sample@gmail.com" value={email}
                ...
                style={{ width: '100%', height: '52px', borderRadius: '12px', border: `1px solid ${error ? '#AE2012' : '#E8E8EA'}`, padding: '0 16px', fontSize: '16px', color: '#414143', outline: 'none', boxSizing: 'border-box' }} />
            ...
            <button onClick={handleEmailLogin} disabled={!isLoginActive || loading}
              style={{ width: '100%', height: '56px', borderRadius: '12px', border: 'none', backgroundColor: isLoginActive ? '#0097FF' : '#C7C7CA', color: '#FFFFFF', fontSize: '18px', fontWeight: '600', cursor: isLoginActive ? 'pointer' : 'not-allowed', marginBottom: '24px', transition: 'background-color 0.2s ease' }}>
